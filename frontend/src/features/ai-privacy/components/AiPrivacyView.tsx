...
import { getAiPrivacyClient, passedCount } from "@/services/ai-privacy";
const keys = createQueryKeyFactory("ai-privacy");
const ctx = { personaId: undefined, organizationId: null, subjectId: null };
export function AiPrivacyView() {
  ...
  const listQuery = useQuery({
    ...
    queryFn: () => getAiPrivacyClient().list(),
    enabled: Boolean(sessionQuery.data),
  });
  const passMutation = useMutation({
    mutationFn: (id: string) => getAiPrivacyClient().markPass(id),
    onSuccess: () => {
      pushFeedback({
        tone: "success",
        title: t(locale, "aiPrivacy", "actionSuccess"),
      });
  ...
  if (!sessionQuery.data && !sessionQuery.isLoading)
    return (
      <main className="mx-auto max-w-4xl p-6" dir={dir} lang={locale}>
        <EmptyState title={t(locale, "aiPrivacy", "signInRequired")} />
      </main>
    );
    ...
    <main className="mx-auto max-w-4xl space-y-6 p-6" dir={dir} lang={locale}>
      <header className="space-y-2">
        ...
        <h1 className="font-display text-3xl font-medium">
          {t(locale, "aiPrivacy", "title")}
        </h1>
        <p className="text-muted text-sm">
          {t(locale, "aiPrivacy", "subtitle")}
        </p>
        <p data-testid="ai-privacy-passed">
          {t(locale, "aiPrivacy", "passedLabel")}: {passedCount(items)}
        </p>
      </header>
      {listQuery.isError ? (
        <ErrorState title={t(locale, "aiPrivacy", "errorTitle")} />
      ) : null}
      <ul
        className="space-y-3"
        aria-label={t(locale, "aiPrivacy", "listLabel")}
        data-testid="ai-privacy-list"
      >
        {items.map((row) => (
          <li
            key={row.id}
          ...
          >
            <div>
              ...
              <p data-testid={`ai-privacy-status-${row.id}`}>{row.status}</p>
            </div>
            {row.status !== "pass" ? (
              <Button type="button" onClick={() => passMutation.mutate(row.id)}>
                {t(locale, "aiPrivacy", "markPass")}
              </Button>
            ) : null}
          </li>
        ))}
