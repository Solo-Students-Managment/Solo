...
import { getOfflineSyncClient, passedCount } from "@/services/offline-sync";
const keys = createQueryKeyFactory("offline-sync");
...
export function OfflineSyncView() {
  ...
  const listQuery = useQuery({
    ...
    queryFn: () => getOfflineSyncClient().list(),
    enabled: Boolean(sessionQuery.data),
  });
  const passMutation = useMutation({
    mutationFn: (id: string) => getOfflineSyncClient().markPass(id),
    onSuccess: () => {
      pushFeedback({
        tone: "success",
        title: t(locale, "offlineSync", "actionSuccess"),
      });
  ...
  if (!sessionQuery.data && !sessionQuery.isLoading)
    return (
      <main className="mx-auto max-w-4xl p-6" dir={dir} lang={locale}>
        <EmptyState title={t(locale, "offlineSync", "signInRequired")} />
      </main>
    );
    ...
    <main className="mx-auto max-w-4xl space-y-6 p-6" dir={dir} lang={locale}>
      <header className="space-y-2">
        ...
        <h1 className="font-display text-3xl font-medium">
          {t(locale, "offlineSync", "title")}
        </h1>
        <p className="text-muted text-sm">
          {t(locale, "offlineSync", "subtitle")}
        </p>
        <p data-testid="offline-sync-passed">
          {t(locale, "offlineSync", "passedLabel")}: {passedCount(items)}
        </p>
      </header>
      {listQuery.isError ? (
        <ErrorState title={t(locale, "offlineSync", "errorTitle")} />
      ) : null}
      <ul
        className="space-y-3"
        aria-label={t(locale, "offlineSync", "listLabel")}
        data-testid="offline-sync-list"
      >
        {items.map((row) => (
          <li
            key={row.id}
          ...
          >
            <div>
              ...
              <p data-testid={`offline-sync-status-${row.id}`}>{row.status}</p>
            </div>
            {row.status !== "pass" ? (
              <Button type="button" onClick={() => passMutation.mutate(row.id)}>
                {t(locale, "offlineSync", "markPass")}
              </Button>
            ) : null}
          </li>
        ))}
