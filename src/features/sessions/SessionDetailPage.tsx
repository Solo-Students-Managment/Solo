...
import { ParentApprovalSection } from '@/features/shared/components/ParentApprovalSection';
import { SessionDetailView } from '@/features/shared/components/SessionDetailView';
...
export function SessionDetailPage() {
  ...
  const canApprove = isParent && parentChildren.some((child) => child.userId === session.studentId);
  ...
  return (
    <div className="space-y-6">
      ...
      />
      {canApprove && (
        <ParentApprovalSection sessionId={session.id} defaultConfirmed={session.parentConfirmed} />
      )}
    </div>
  );
}
