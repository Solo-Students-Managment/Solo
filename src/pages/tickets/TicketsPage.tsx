...
export function TicketsPage() {
  ...
  const activeTicket = visibleTickets.find((ticket) => ticket.id === (selectedId ?? visibleTickets[0]?.id))
  ...
  const handleCreate = (event: React.FormEvent) => {
    ...
    try {
      ...
      setShowCreate(false)
      setNewTicket({ title: '', description: '', priority: 'medium', studentId: '' })
          ...
          <CardContent>
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label>اولویت</Label>
                  <Select value={newTicket.priority} onValueChange={(v) => setNewTicket({ ...newTicket, priority: v as typeof newTicket.priority })}>
                ...
                {user.role === 'teacher' && (
                  <div className="space-y-2">
                    ...
                    <Select value={newTicket.studentId} onValueChange={(v) => setNewTicket({ ...newTicket, studentId: v })}>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div className="space-y-2">
          ...
          {visibleTickets.length === 0 && (
            <p className="rounded-xl border p-6 text-center text-sm text-muted-foreground">تیکتی وجود ندارد</p>
          )}
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3 border-t pt-4">
                ...
                <form onSubmit={handleComment} className="flex gap-2">
                  ...
                  />
                  <Button type="submit" className="self-end">ارسال</Button>
                </form>
              </div>
            </CardContent>
          </Card>
        ) : (
          <div className="flex min-h-[300px] items-center justify-center rounded-xl border text-sm text-muted-foreground">
            تیکتی انتخاب نشده
          </div>
        )}
      </div>
